...
def connected_to_server(server_type: str, status: str):
    """
    Prints a 'Connected to Server' section for beacon, full width,
    with two columns: 
    ...
    panel = Panel(
        table, 
        title="[bold cyan]beacon - Server Connection[/bold cyan]",
        border_style="cyan",
...
def call_end(result: Any, llm_model: str, response_format: str, start_time: float, end_time: float, usage: dict, debug: bool = False):
    ...
    panel = Panel(
        table,
        title="[bold white]beacon - Call Result[/bold white]",
        border_style="white",
        expand=True,
        width=70
    )
...
def agent_end(result: Any, llm_model: str, response_format: str, start_time: float, end_time: float, usage: dict, tool_count: int, context_count: int, debug: bool = False):
    ...
    panel = Panel(
        table,
        title="[bold white]beacon - Agent Result[/bold white]",
        border_style="white",
        expand=True,
        width=70
    )
...
def agent_total_cost(total_input_tokens: int, total_output_tokens: int, total_time: float, llm_model: str):
    ...
    panel = Panel(
        table,
        title="[bold white]beacon - Agent Total Cost[/bold white]",
        border_style="white",
        expand=True,
        width=70
    )
...
def agent_retry(retry_count: int, max_retries: int):
    ...
    panel = Panel(
        table,
        title="[bold yellow]beacon - Agent Retry[/bold yellow]",
        border_style="yellow",
        expand=True,
        width=70
    )
